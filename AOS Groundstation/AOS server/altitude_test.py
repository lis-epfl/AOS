...
drone_id = 1
altitude_increment = 15  # meters to increase altitude
...
try:
    ...
    waypoint_data = f"{latitude}:{longitude}:{str(new_altitude)}:{heading}:2:{waypoint_num}:5:{str(new_gimbal_pitch)}:{gimbal_yaw}:90"
